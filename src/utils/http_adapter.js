...
import createError from '../../node_modules/axios/lib/core/createError';
import enhanceError from '../../node_modules/axios/lib/core/enhanceError';

const { http: httpFollow, https: httpsFollow } = follow;
  ...
  return new Promise((resolve, reject) => {
    ...
    if (data && !utils.isStream(data)) {
      if (Buffer.isBuffer(data)) {
        ...
        errMsg = 'Data after transformation must be a string, an ArrayBuffer, ';
        errMsg += 'a Buffer, or a Stream';
        return reject(createError(errMsg, config));
      }
    ...
    if (proxy) {
      ...
      options.path += `${(parsed.port ? `:${parsed.port}` : '')}`;
